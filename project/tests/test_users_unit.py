...
def test_add_user(test_app, monkeypatch):
    ...
    resp = client.post(
        "/users",
        data=json.dumps({"username": "mk", "email": "mk@email.com"}),
        content_type="application/json",
    )
    data = json.loads(resp.data.decode())
    assert resp.status_code == 201
    assert "mk@email.com was added!" in data["message"]
...
def test_add_user_invalid_json_key(test_app, monkeypatch):
    client = test_app.test_client()
    resp = client.post(
        "/users",
...
def test_add_user_duplicate_email(test_app, monkeypatch):
    ...
    client.post(
        "/users",
        data=json.dumps({"username": "m", "email": "ml@email.io"}),
        content_type="application/json",
    )
    resp = client.post(
        "/users",
        data=json.dumps({"username": "m", "email": "m@email.io"}),
        content_type="application/json",
    )
    data = json.loads(resp.data.decode())
    assert resp.status_code == 400
...
def test_single_user(test_app, monkeypatch):
    ...
    assert "j" in data["username"]
    assert "j@email.io" in data["email"]
...
def test_all_users(test_app, monkeypatch):
    ...
    assert resp.status_code == 200
    assert len(data) == 2
    assert "m" in data[0]["username"]
    assert "m@mail.io" in data[0]["email"]
    assert "f" in data[1]["username"]
    assert "f@notreal.com" in data[1]["email"]
...
def test_remove_user(test_app, monkeypatch):
    ...
    def mock_get_user_by_id(user_id):
        d = AttrDict()
        d.update(
            {"id": 1, "username": "user-to-be-removed", "email": "remove-me@email.io"}
        )
        return d
...
def test_update_user(test_app, monkeypatch):
    ...
    def mock_get_user_by_id(user_id):
        d = AttrDict()
        d.update({"id": 1, "username": "me", "email": "me@mail.com"})
        return d
    ...
    resp_one = client.put(
        "/users/1",
        data=json.dumps({"username": "me", "email": "me@mail.com"}),
        content_type="application/json",
    )
    ...
    assert "me@mail.com" in data["email"]
